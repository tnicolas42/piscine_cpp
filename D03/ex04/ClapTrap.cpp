...
_type("CLAPTRAP") {
    srand(time(NULL));
    std::cout << "[" << _type << "] new ClapTrap: " << _name << std::endl;
}
...
bool ClapTrap::atk(const std::string atkName, int damage, int energy, const std::string &target) {
    if (_energyPoint - energy < 0) {
        std::cout << "[" << _type << "][" << getType() << "][" << _name << "][ERROR]: not enouth energy (" <<
            _energyPoint << ") for " << atkName << ". needed " << energy << std::endl;
        return false;
    }
    _energyPoint -= energy;
    std::cout << "[" << _type << "][" << getType() << "][" << _name << "] atk " << atkName << " on " <<
...
void ClapTrap::takedamage(uint32_t amount) {
    ...
    _hitPoint -= amount;
    std::cout << "[" << _type << "][" << getType() << "][" << _name << "] take " << amount << " points of damage. Now at " <<
        _hitPoint << "." << std::endl;
}
void ClapTrap::beRepaired(uint32_t amount) {
    ...
    _hitPoint += amount;
    std::cout << "[" << _type << "][" << getType() << "][" << _name << "] heath of " << amount << " lifepoint. Now at " <<
        _hitPoint << "." << std::endl;
}
